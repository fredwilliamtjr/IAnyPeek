<div align="center">
  <img src="docs/icon.png" width="160" alt="IAnyPeek icon">

  <h1>IAnyPeek</h1>

  <p><strong>Uma agenda simples e sincronizável para abrir máquinas do AnyDesk com um clique.</strong></p>
...
Pra quem cuida de três, quatro ou cinco máquinas pessoais, a falta de uma agenda vira fricção boba: copiar ID em nota, lembrar qual Mac é qual, abrir o AnyDesk manualmente, conferir número, clicar, repetir.

## ✨ A solução

IAnyPeek é uma agenda discreta para AnyDesk. Ele fica na barra de menu do macOS ou no system tray do Windows; ao clicar no ícone, mostra suas máquinas salvas. Um clique na máquina chama `anydesk:<id>` e abre o AnyDesk direto na conexão certa.

O app não tenta substituir o AnyDesk. Ele só entrega a camada que ficou faltando: uma lista simples, sincronizável e rápida.

## 🎯 Features

- 🖥️ **Agenda de máquinas** — nome amigável, ID do AnyDesk, tags e notas
- ⚡ **Abrir com um clique** — chama o protocolo `anydesk:<id>` direto pelo sistema
...
4. Cadastre suas máquinas com nome e ID do AnyDesk
...
|---|---|
| `main.js` | Tray, janelas, seleção de provedor, leitura/gravação do catálogo e abertura do AnyDesk |
...
Se o serviço escolhido já tiver máquinas, o IAnyPeek carrega essa lista. Se estiver vazio, ele mantém a lista atual em tela e só grava no novo serviço quando você clicar em **Sincronizar**.
...
```bash
git clone https://github.com/fredwilliamtjr/IAnyPeek.git
cd IAnyPeek
npm install
npm start
```
...
```text
release/mac-universal/IAnyPeek.app
```

## 🔒 Segurança

- O IAnyPeek guarda apenas IDs do AnyDesk, nomes, notas e tags
- Senhas de acesso desacompanhado devem ficar no AnyDesk ou num cofre dedicado
