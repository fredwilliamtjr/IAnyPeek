...
const elements = {
  ...
  anydeskId: document.getElementById('anydesk-id'),
...
async function init() {
  bindEvents();
  render(await window.ianyPeek.getState());
  window.ianyPeek.onStateChanged(render);
}
...
function bindEvents() {
  ...
  document.getElementById('reload').addEventListener('click', async () => {
    ...
    render(await window.ianyPeek.syncNow());
  });
  elements.syncProvider.addEventListener('change', async (event) => {
    if (!event.target.value) return;
    render(await window.ianyPeek.setSyncProvider(event.target.value));
  });
  elements.launchAtLogin.addEventListener('change', async (event) => {
    render(await window.ianyPeek.setLaunchAtLogin(event.target.checked));
  });
  ...
  elements.deleteDevice.addEventListener('click', async () => {
    ...
    render(await window.ianyPeek.deleteDevice(editingDevice.id));
...
function renderDeviceList() {
  ...
  const devices = state.book.devices.filter((device) => {
    const haystack = `${device.name} ${device.anydeskId} ${(device.tags || []).join(' ')}`.toLowerCase();
  ...
  if (!devices.length) {
    ...
    empty.textContent = state.book.devices.length ? 'Nenhuma máquina combina com a busca.' : 'Cadastre sua primeira máquina para ela aparecer no menu do IAnyPeek.';
  ...
  for (const device of devices) {
    ...
    info.innerHTML = `
      ...
      <div class="device-meta">${formatAnyDeskId(device.anydeskId)}</div>
    ...
    connect.addEventListener('click', () => window.ianyPeek.connectDevice(device.anydeskId));
...
function openDeviceDialog(device = null) {
  ...
  elements.anydeskId.value = device?.anydeskId || '';
...
async function saveDevice(event) {
  ...
  const device = {
    ...
    anydeskId: elements.anydeskId.value,
    tags: elements.deviceTags.value,
    notes: elements.deviceNotes.value
  };

  try {
    render(await window.ianyPeek.upsertDevice(device));
    elements.dialog.close();
  } catch (error) {
