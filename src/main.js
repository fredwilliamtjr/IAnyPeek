...
function normalizeAnyDeskId(value) {
...
function sanitizeBook(raw) {
  ...
  return {
    ...
    devices: devices
      .map((device) => {
        const anydeskId = normalizeAnyDeskId(device.anydeskId);
        const name = String(device.name || '').trim();
        if (!anydeskId || !name) return null;
...
function openAnyDesk(rawId) {
  const id = normalizeAnyDeskId(rawId);
  if (!id) return;
  shell.openExternal(`anydesk:${id}`);
}
...
function upsertDevice(device) {
  ...
  if (!name || !anydeskId) {
    throw new Error('Nome e ID do AnyDesk são obrigatórios.');
  }
...
ipcMain.handle('device:connect', (_event, anydeskId) => openAnyDesk(anydeskId));
